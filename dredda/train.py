...
class DualBranchDATrainer(object):
    ...
    def __init__(
        ...
        self.dual_branch_optimizer=optim.Adam([{'params':dual_branch_parameter}],lr=self.target_lr)
        self.other_optimizer=optim.Adam([{'params':other_parameter,}],lr=self.other_lr)
    ...
    def fit(
        ...
        iprint("=============== Training ===============")
        iprint("Length of dataloaders","source:",len(dataloader_source),"target:", len(dataloader_target),ident=0)
        iprint("Parameters:","alpha = %.4f, domain_adv_coeff = %.4f, ddc_coeff = %.4f, ddc_features = %s"%(self.alpha,self.domain_adv_coeff,self.ddc_coeff,self.ddc_features),ident=0)
        ...
        iprint('Accuracy on %s dataset: %f' % (self.source_dataset_name, accu_s),ident=1)
        ...
        for epoch in range(n_epochs):
            ...
            for i in range(len_dataloader):
                ...
                s_class_output,s_domain_output,s_ddc_features=self.model(s_data,"source",use_dual_branch=epoch>self.dual_training_epoch,alpha=self.alpha,return_ddc_features=self.ddc_features)
                _,t_domain_output,t_ddc_features=self.model(t_data,"target",use_dual_branch=epoch>self.dual_training_epoch,alpha=self.alpha,return_ddc_features=self.ddc_features)
            ...
            iprint_dict(score_source_train,ident=2,keys=["class_accuracy","class_loss","domain_loss","domain_accuracy"])
            ...
            if X_source_val is not None:
                ...
                iprint_dict(score_source_val,ident=2,keys=["class_accuracy","class_loss","domain_loss","domain_accuracy"])
                iprint("confusion matrix:",ident=2)
