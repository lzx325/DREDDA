...
def get_CRISPRi_LINCS_dataset(feature_selection):
    ...
    if feature_selection == "MI_ae_expectation_1000":
        level5_fs_table_fp = os.path.join(
            root_dir, "level5_ae_expectation_logtrans_fs_1000-368.hdf"
        )
        CRISPRi_fs_table_fp = os.path.join(
            root_dir, "CRISPRi_ae_expectation_logtrans_fs_1000-368.hdf"
        )
    else:
    ...
    cluster_labels_df = pd.read_csv(
        os.path.join(root_dir, "cells_batches_cluster.txt"), sep=",", index_col=0
    )

    CRISPRi_fs_values_sample_df = CRISPRi_fs_values_df.copy()
    level5_fs_sample_df = level5_fs_df.sample(
        20000, axis=1, random_state=123
    )  # to keep the two datasets of comparable size

    icindex = [
        CRISPRi_fs_values_df.columns.get_loc(i)
        for i in CRISPRi_fs_values_sample_df.columns
    ]
